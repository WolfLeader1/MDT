...
huntingZone:onPlayerInOut(function(isPointInside, point)
    ...
    print("[Dispatch] Am I in hunting zone? ", inHuntingZone)
end)
...
RegisterCommand("testnotif2", function()
    AlertFight()
end, false)

RegisterCommand("testnotif3", function()
    AlertCheckLockpick()
end, false)

RegisterCommand("testnotif4", function()
    AlertCheckLockpick()
end, false)]]
...
CreateThread(function() -- Use this only if you think/plan on restarting the resource often.
    while PlayerData['cid'] == nil do
      PlayerData = exports['envyrp']:GetPlayerData()
      Wait(5000)
    end
    return
  end)

RegisterNetEvent('envyrp:playerSpawned') -- Use this to grab player info on spawn.
AddEventHandler('envyrp:playerSpawned', function(sentData) PlayerData = sentData end)

RegisterNetEvent('envyrp:updateinfo')
AddEventHandler('envyrp:updateinfo', function(toChange, targetData) 
    PlayerData[toChange] = targetData
end)

RegisterNetEvent('envyrp:doLogout') -- Use this to logout.
AddEventHandler('envyrp:doLogout', function(sentData) 
    PlayerData = {}
    currentCallSign = ""
...
function ArmedPlayer() -- When aiming weapon.
    ...
    TriggerServerEvent('dispatch:svNotify', {
        ...
        job = {"lspd","bcso","pa","sahp","sapr","sasp","doc"}
    })

end

RegisterNetEvent('erp-dispatch:gunshotAlert')
AddEventHandler('erp-dispatch:gunshotAlert', function(sentCoords)
    if sentCoords then
        if (PlayerData.job.isPolice) and PlayerData.job.duty == 1 then
			...
			AddTextComponentString('10-60 Shots Fired')              -- to 'supermarket'
...
AddEventHandler('erp-dispatch:manageNotifs', function(sentSetting)
    ...
    if wantedSetting == "on" then
        ...
        exports['mythic_notify']:SendAlert('inform', 'Dispatch enabled.')
    elseif wantedSetting == "off" then
        disableNotis = true
        disableNotifSounds = true
        exports['mythic_notify']:SendAlert('inform', 'Dispatch disabled.')
    elseif wantedSetting == "mute" then
        disableNotis = false
        disableNotifSounds = true
        exports['mythic_notify']:SendAlert('inform', 'Dispatch muted.')
    else
        exports['mythic_notify']:SendAlert('inform', 'Please choose to have dispatch as "on", "off" or "mute".', 5000)
    end
end)
...
function AlertGunShot(isHunting, sentWeapon) -- Check for automatic, change priority to 1
    if KnownWeapons[sentWeapon] then
        ...
        TriggerServerEvent('erp-dispatch:gunshotAlert', currentPos)

        local job = {"lspd","bcso","pa","sahp","sapr","sasp","doc"}
        ...
        if math.random(1, 10) > 3 and not isInVehicle then
            CreateThread(function()
                ...
                if IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    TriggerServerEvent('erp-dispatch:gunshotAlert', newPos)
                end
                return
            end)
        end
...
RegisterCommand('911', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        local plyData = exports['envyrp']:GetPlayerData()
        local locationInfo = GetStreetAndZone()
        local gender = IsPedMale(playerPed)
        local currentPos = GetEntityCoords(playerPed)

        TriggerEvent('dp:playEmote', "phone")
		local rfl = exports['erp-progressbar']:taskBar(math.random(2500, 4000), "Dialing 911")
        ...
        if rfl == 100 then
            TriggerServerEvent('dispatch:svNotify', {
                ...
                job = {"lspd","bcso","pa","sahp","sapr","sasp","doc","ambulance", "cmmc"},
                information = msg
            })
        end
    else
        exports['mythic_notify']:SendAlert('inform', 'Please put a reason after the 911')
    end
end)
...
RegisterCommand('311', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        ...
        TriggerEvent('dp:playEmote', "phone")
		local rfl = exports['erp-progressbar']:taskBar(math.random(2500, 4000), "Dialing 311")
        TriggerEvent('dp:cancelEmote')
        if rfl == 100 then
            TriggerServerEvent('dispatch:svNotify', {
                dispatchCode = "311",
        ...
        exports['mythic_notify']:SendAlert('inform', 'Please put a reason after the 311')
    end
end)
