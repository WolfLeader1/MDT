...
function EnableGUI(enable)
    ...
    SendNUIMessage({ type = "show", enable = enable, job = exports["isPed"]:isPed("myjob").name })
    isOpen = enable
...
AddEventHandler('erp_mdt:newBulletin', function(ignoreId, sentData, job)
    ...
    if job == 'police' and exports["isPed"]:isPed("myjob").isPolice then
        SendNUIMessage({ type = "newBulletin", data = sentData })
    elseif job == exports["isPed"]:isPed("myjob").name then
        SendNUIMessage({ type = "newBulletin", data = sentData })
    end
end)
...
AddEventHandler('erp_mdt:getActiveUnits', function(lspd, bcso, sahp, sasp, doc, sapr, pa, ems)
...
RegisterNUICallback("dispatchNotif", function(data, cb)
    ...
    TriggerEvent('erp_phone:sendNotification', {img = info['profilepic'], title = "Dispatch ("..info['name']..")", content = info['message'], time = 7500, customPic = true })
    cb(true)
end)
