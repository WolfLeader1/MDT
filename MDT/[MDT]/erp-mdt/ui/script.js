    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://erp-mdt/getWarrants', JSON.stringify({}));
                $("#warrants").addClass("nav-active");
            } else if (page == "Bolos") {
                $.post('http://erp-mdt/getBolos', JSON.stringify({}));
        ...
        closeMDT() {
            $.post('http://erp-mdt/close', JSON.stringify({}));
        },
        logOff() {
            $.post('http://erp-mdt/logOff', JSON.stringify({}));
        },
        logIn() {
            $.post('http://erp-mdt/logIn', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://erp-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://erp-mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://erp-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://erp-mdt/performReportSearch', JSON.stringify({
        ...
        BoloSearch() {
            if (this.bolo_search) {
                ...
                this.bolo_results.query = this.bolo_search
                $.post('http://erp-mdt/performBoloSearch', JSON.stringify({
                    query: this.bolo_search
                }));
        
                this.bolo_results.results = false;
                this.bolo_selected = {
                    id: null,
                    date: null,
                    name: null,
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://erp-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://erp-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        ToggleBoloEdit() {
            if (this.bolo_edit.enable) {
                this.bolo_edit.enable = false;
                this.bolo_edit.data = {}
            } else {
                this.bolo_edit.enable = true;
                this.bolo_edit.data.plate = this.bolo_selected.plate;
                this.bolo_edit.data.owner = this.bolo_selected.owner;
                this.bolo_edit.data.individual = this.bolo_selected.individual;
            }
            return;
        },
        DeleteSelectedReport() {
            $.post('http://erp-mdt/deleteReport', JSON.stringify({
        ...
        DeleteSelectedBolo() {
            $.post('http://erp-mdt/deleteBolo', JSON.stringify({
                id: this.bolo_selected.id,
            }));
            this.changePage("Search Bolos");
            this.bolo_selected = {
                id: null,
                date: null,
                name: null,
        ...
        SaveReportEditChanges() {
            $.post('http://erp-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        SaveBoloEditChanges() {
            $.post('http://erp-mdt/saveBoloChanges', JSON.stringify({
                id: this.bolo_selected.id,
                plate: this.bolo_edit.data.plate,
                owner: this.bolo_edit.data.owner,
                individual: this.bolo_edit.data.individual,
            }));
        
            this.bolo_selected.plate = this.bolo_edit.data.plate;
            this.bolo_selected.owner = this.bolo_edit.data.owner;
            this.bolo_selected.individual = this.bolo_edit.data.individual;
            this.ToggleBoloEdit();
            return;
        },
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://erp-mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://erp-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        Unimpound(result) {
            $.post('http://erp-mdt/unimpoundMofoka', JSON.stringify({
                plate: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://erp-mdt/submitNewWarrant', JSON.stringify({
        ...
        SubmitNewBolo() {
            ...
            $.post('http://erp-mdt/submitNewBolo', JSON.stringify({
                title: this.bolo_new.title,
                plate: this.bolo_new.plate,
                owner: this.bolo_new.owner,
                individual: this.bolo_new.individual,
                reason: this.bolo_new.reason,
                officersinvolved: this.bolo_new.officersinvolved,
            ...
            this.bolo_results.results = false;
            this.bolo_results.query = "";
            return;
        },
        DeleteSelectedWarrant() {
            $.post('http://erp-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://erp-mdt/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://erp-mdt/sentencePlayer', JSON.stringify({
    ...
    computed: {
        ...
        filtered_bolos() {
            return this.bolos.filter(bolo => {
                if (
                    bolo.title.toLowerCase().search(this.bolo_search.toLowerCase()) != -1
                    )
                    return bolo;
            })
        }
...
document.onreadystatechange = () => {
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            ...
            } else if (event.data.type == "sendOfficerName") {
                mdtApp.officer.name = event.data.name;
            } else if (event.data.type == "returnedReportMatches") {
                mdtApp.report_results.results = event.data.matches;
            } else if (event.data.type == "returnedBoloMatches") {
                mdtApp.bolo_results.results = event.data.matches;
            ...
            } else if (event.data.type == "imoffduty") {
                mdtApp.changePage("LogIn");
            } else if (event.data.type == "completedWarrantAction") {
                mdtApp.changePage("Warrants");
...
function BoloTimer() {
    var timer = setInterval(function() {
        for (var key in mdtApp.bolos) {
            var bolo = mdtApp.bolos[key]
            var now = new Date().getTime();
            var expire_time = new Date(bolo.expire).getTime();
            var t = expire_time - now;
            if (t >= 0) {
                var days = Math.floor(t / (1000 * 60 * 60 * 24));
                var hours = Math.floor((t % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
